...
Cypress.Commands.add( 'resetGeneralSettingTabs', () => {
	cy.exec(
		`npx wp-env run cli wp db query < tests/cypress/fixtures/clearStoreInfoData.sql`
	);
} );

Cypress.Commands.add( 'enableComingSoon', () => {
	cy.visit( '/wp-admin/admin.php?page=bluehost#/settings' );
	cy.get( '[aria-label="Coming Soon Page"]' ).then( ( $comingSoon ) => {
		if ( $comingSoon.attr( 'aria-checked' ) == 'false' ) {
			cy.wrap( $comingSoon ).click();
		}
	} );
	cy.reload();
...
Cypress.Commands.add( 'setCustomerData', () => {
	const cdata = JSON.stringify( customerData ).replaceAll( ',', '\\,' );
	cy.exec(
		`npx wp-env run cli wp option update bh_cdata_guapi '${ cdata }'`
	);
} );
