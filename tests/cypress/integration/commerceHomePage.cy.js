...
const homeUrl = '/wp-admin/admin.php?page=bluehost#/home/store/general';
...
describe('As a wp-admin user, I want to ', function () {
  ...
  it('verify the launch pad banner on top of the home page when site status is not live', () => {
    cy.enableComingSoon();
    cy.get('div.nfd-ecommerce-banner', {
      timeout: customCommandTimeout,
    }).as('homePageBanner');
    cy.reload();
  ...
  it('add my Store address from "General Setting" "Store Info" card', () => {
    ...
    cy.get('input[name=woocommerce_store_address]').type(
      this.data.store_address.usa.address1
    );
    cy.get('[name=woocommerce_store_city]').type(
      this.data.store_address.usa.city
    );
    cy.get('input[name=woocommerce_store_postcode]').type(
      this.data.store_address.usa.zipcode
    );
    cy.get('select[name=country]').select(this.data.store_address.usa.country);
    cy.get('select[name=state]').select(this.data.store_address.usa.state);
    cy.get('[name=woocommerce_email_from_address]').type(
      this.data.store_address.usa.email
    );
    cy.get('[name=woocommerce_currency]').select(
      this.data.store_address.usa.currency
    );
    ...
    cy.get('input#woocommerce_store_address', {
      timeout: customCommandTimeout,
    }).should('have.value', this.data.store_address.usa.address1);
    ...
    cy.get('select[name=woocommerce_default_country] option:selected').should(
      'have.text',
      `${
        this.data.store_address.usa.country +
        ' — ' +
        this.data.store_address.usa.state
      }`
    );
  ...
  it('see the option to link my existing payment method in General Setting', () => {
    ...
    if (this.data.paypal.environment === 'sandbox') {
      cy.get('@paypalRadioButtons').eq(1).click();
    } else {
      cy.get('@paypalRadioButtons').eq(0).click();
    }
    if (this.data.paypal.payment_action.toLowerCase().includes('sale')) {
  ...
  it('link my existing shippo account in General Setting', () => {
    ...
    if (this.data.shippo.environment.toLowerCase().includes('sandbox')) {
    ...
    cy.iframe()
      ...
      .clear()
      .type(this.data.shippo.test_api);
    ...
    cy.iframe()
      ...
      .clear()
      .type(this.data.shippo.sender_name);
    ...
    cy.iframe()
      ...
      .find('input')
      .clear()
      .type(this.data.shippo.sender_company);
    ...
    cy.iframe()
      ...
      .find('input')
      .clear()
      .type(this.data.shippo.email);

    if (this.data.shippo.use_woocommerce_address) {
      cy.iframe()
        .find('[type=checkbox]')
        .then(($element) => {
    ...
    cy.get('input[type=radio][checked=checked]', {
      timeout: customCommandTimeout,
    }).should('include.value', this.data.shippo.environment);
    cy.get('#yith_shippo_sandbox_token').should(
      'have.value',
      this.data.shippo.test_api
    );
