...
describe( 'As a wp-admin user, I want to ', function () {
	...
	it.only( 'verify "WooCommerce is not installed!" model displaying when WooCommerce plugin is not install or active', () => {
	...
	it.only( 'see the option to link my existing payment method in General Setting', () => {
		cy.get( 'button.nfd-ecommerce-card' )
			.find( '.nfd-ecommerce-card-title' )
			.contains( 'Payments' )
			.click();

		cy.get( '.nfd-ecommerce-modal-plugin-install-failed-content' )
			.findByText( 'Okay' )
			.click();
		cy.log( cy.intercept( '**status&_locale=user' ) );
		cy.wait( 60000 );
		cy.reload();
		cy.log( cy.intercept( '**status&_locale=user' ) );
	...
	it( 'create , edit, and manage gift card', () => {
		...
		cy.url().should(
			'include',
			'edit.php?post_type=gift_card&yith-plugin'
		);
	} );
	...
	it( 'Launch My Store', () => {
		...
		cy.get( 'div.site-status-banner', {
			...
			cy.wrap( $element.find( 'h2' ).text() ).should(
				'eq',
				'Ready to go live?'
			);
		...
		cy.get( '@analytics' )
			.find( 'h2' )
			.should( 'have.text', 'Store Analytics' );
		cy.get( '@analytics' )
		...
		cy.get( '@analytics' )
			.find( 'a' )
			.should( 'have.text', 'View Analytics' );
	} );
} );
