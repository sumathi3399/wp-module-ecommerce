...
describe( 'Verify ', function () {
	Object.values( dimensions ).map( ( key, index ) => {
		const deviceName = Object.keys( dimensions )[ index ];
		...
		it( `store info are displaying properly in done state in ${ deviceName }`, () => {
			...
			cy.get( 'select[name=state]' ).select(
			...
			cy.contains( 'Store Info', { timeout: 10000 } )
				.parent()
				.find( '.nfd-ecommerce-taskCompleted-image', {
					timeout: 10000,
				} )
				.should( 'exist' );
		...
		it( `Payments Card fields are properly allign in ${ deviceName }`, () => {
			...
			cy.iframe()
				...
				.then( ( $element ) => {
					const formWidth = $element.outerWidth();
					expect( formWidth ).be.lessThan( key.width );
					expect(
						$element.find( 'input:visible' ).outerWidth()
					).be.lessThan( formWidth );
					expect(
						$element.find( 'p:visible' ).outerWidth()
					).be.lessThan( formWidth );
		...
		it( `Tax info cards fields are properly allign in ${ deviceName }`, () => {
			...
			cy.get( '[role="document"] p' ).then( ( $para, index ) => {
				for ( let index = 0; index < $para.length; index++ ) {
					if ( index != 2 ) {
						expect( $para.css( 'text-align' ) ).to.be.eq(
							'center'
						);
					}
		...
		it.only( `Product and Services cards are displaying properly in ${ deviceName }`, () => {
			...
			cy.get( 'button.nfd-ecommerce-card', { timeout: 50000 } ).each(
				( $element ) => {
					...
					cy.get( '.nfd-ecommerce-card-title' ).each(
						( title, index ) => {
							if (
								title.text().includes( 'How to add products' )
							) {
								cy.wrap( title.css( 'text-align' ) ).should(
									'eq',
									'left'
								);
							} else {
			...
			cy.get( 'button.nfd-ecommerce-card', { timeout: 50000 } ).each(
				( $element ) => {
					...
					cy.wrap( $element.find( 'span' ) )
						...
						.should( 'be.lt', $element.outerWidth() );
					cy.wrap(
						$element.find( 'span' ).css( 'text-align' )
					).should( 'eq', 'left' );
				}
			);
		} );
